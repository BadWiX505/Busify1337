...
export default function StudentCard({ data }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-950 p-6 rounded-lg shadow-lg max-w-md mx-auto relative overflow-hidden">
      <div
        className="absolute w-100 h-50 bg-[#F0FFF7] rounded-full"
        style={{
          width: "200px",
          height: "200px",
          top: "-20%",
      ...
      <div className="grid grid-cols-2 gap-4 mt-4">
        ...
        <div className="relative z-3">
          ...
          <p className="font-medium">
            {formatShortReadableDate(data.bookedAt)}
          </p>
        </div>
      </div>
        ...
        <Dialog onOpenChange={changeDialogeMode}>
          <DialogTrigger asChild>
            <Button
              size="sm"
              variant="outline"
              className=" bg-transparent hover:bg-[rgb(240,255,247)]"
            >
              Get ticket
            </Button>
          </DialogTrigger>
          {isDialogOpen && <StudentTicket idBooking={data.id_Booking} />}
        </Dialog>
      </div>
    </div>
  );
